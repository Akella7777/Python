...
if len(berries) != bushes:
    print(f"{colors.ERROR} Количество введенных значений не совпадает с количеством кустов. Введите корректные данные {colors.END}")
else:
    for i in range(bushes):
        berries_sum = berries[i - 1] + berries[i] + berries[i + 1 if i < bushes - 1 else 0]
